    ...
    public class LuaState : IDisposable
    {
        ...
        public IntPtr L
        {
            get
            {
                ...
                if (!isMainThread())
                {
                    SluaLogger.LogError("Can't access lua in bg thread");
                    throw new Exception("Can't access lua in bg thread");
                }

                if (l_ == IntPtr.Zero)
                {
                    SluaLogger.LogError("LuaState had been destroyed, can't used yet");
...
";
        ...
        public void Close()
        {
            if (L != IntPtr.Zero)
            {
                ...
                {
                    SluaLogger.Log("Finalizing Lua State.");
        ...
        public object doFile(string fn)
        {
            ...
            if (bytes == null)
            {
                SluaLogger.LogError(string.Format("Can't find {0}", fn));
                return null;
            }
